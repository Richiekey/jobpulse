...
class KulaAdapter(BaseAdapter):
    platform = ATSPlatform.KULA

    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch jobs from Kula career site."""
        url = f"https://jobs.kula.ai/{company_identifier}"
        try:
            ...
            for link in links:
                ...
                if title:
                    jobs.append({
                        "id": href.split("/")[-1],
                        "title": title,
                        "url": href if href.startswith("http") else f"https://jobs.kula.ai{href}"
                    })
            return jobs
        except Exception as e:
            logger.warn("kula_fetch_jobs_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        job_url = raw_job.get("url") or f"https://jobs.kula.ai/{company_identifier}"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://jobs.kula.ai/{company_identifier}",
