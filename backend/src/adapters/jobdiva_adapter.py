...
class JobDivaAdapter(BaseAdapter):
    platform = ATSPlatform.JOBDIVA

    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch jobs from JobDiva staffing portal."""
        url = f"https://www1.jobdiva.com/portal/?a={company_identifier}"
        try:
            ...
            jobs = []
            links = soup.select("a[href*='jobdetails.jsp'], a[href*='jobid=']")
            for link in links:
                href = link.get("href", "")
                ...
                if title:
                    jobs.append({
                        "id": href,
                        "title": title,
                        "url": href if href.startswith("http") else f"https://www1.jobdiva.com/portal/{href}"
                    })
            return jobs
        except Exception as e:
            logger.warn("jobdiva_fetch_jobs_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        job_url = raw_job.get("url") or f"https://www1.jobdiva.com/portal/?a={company_identifier}"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://www1.jobdiva.com/portal/?a={company_identifier}",
