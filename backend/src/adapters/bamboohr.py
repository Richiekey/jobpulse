...
class BambooHRAdapter(BaseAdapter):
    ...
    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch job postings for a company from BambooHR careers endpoint."""
        url = f"https://{company_identifier}.bamboohr.com/careers/list"
        try:
            # Try json format first
            response = await self.client.get(f"{url}?json=true")
            ...
            for link in links:
                ...
                if parts and parts[-1].isdigit():
                    ...
                    jobs.append({
                        ...
                        "url": href if href.startswith("http") else f"https://{company_identifier}.bamboohr.com{href}"
                    })
            return jobs
        except Exception as e:
            logger.warn("bamboohr_fetch_jobs_error", company=company_identifier, error=str(e))
            return []

    async def fetch_job_details(self, company_identifier: str, job_id: str) -> Optional[Dict[str, Any]]:
        url = f"https://{company_identifier}.bamboohr.com/careers/{job_id}"
        try:
            response = await self.client.get(url)
            if response.status_code != 200:
                return None
            soup = BeautifulSoup(response.text, "lxml")
            desc_el = soup.select_one(".BambooJobDescription, .description, main")
            return {
                "id": job_id,
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        title = (raw_job.get("jobOpeningName") or raw_job.get("title") or "").strip()

        loc_data = raw_job.get("location") or {}
        city = loc_data.get("city") if isinstance(loc_data, dict) else None
        country = loc_data.get("country") if isinstance(loc_data, dict) else None
        raw_loc = loc_data.get("name") if isinstance(loc_data, dict) else (loc_data if isinstance(loc_data, str) else "")
        parsed_loc = parse_location(raw_loc or ", ".join(filter(None, [city, country])))

        dept = raw_job.get("department")
        dept_name = dept.get("label") if isinstance(dept, dict) else (dept if isinstance(dept, str) else None)
        ...
        job_url = raw_job.get("url") or f"https://{company_identifier}.bamboohr.com/careers/{job_id}"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://{company_identifier}.bamboohr.com/careers",
