...
class PinpointAdapter(BaseAdapter):
    platform = ATSPlatform.PINPOINT

    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch jobs from Pinpoint JSON API endpoint."""
        url = f"https://{company_identifier}.pinpointhq.com/postings.json"
        try:
            ...
            logger.warn("pinpoint_fetch_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        loc_str = raw_job.get("location_name") or raw_job.get("location")
        parsed_loc = parse_location(loc_str)

        emp_type = normalize_employment_type(raw_job.get("employment_type") or title) or normalize_employment_type(desc)
        salary_info = parse_salary(desc) if desc else {"min": None, "max": None, "currency": None, "period": None}
        job_url = raw_job.get("url") or f"https://{company_identifier}.pinpointhq.com/postings/{job_id}"
