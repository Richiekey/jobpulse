...
class TaleoAdapter(BaseAdapter):
    ...
    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        ...
        url = f"https://{company_identifier}.taleo.net/careersection/default/jobsearch.ftl"
        try:
            ...
            jobs = []
            links = soup.select("a[href*='jobdetail.ftl']")
            for link in links:
                title = link.get_text(strip=True)
                href = link.get("href", "")
                if title:
                    jobs.append({"id": href, "title": title, "url": f"https://{company_identifier}.taleo.net{href}"})
            return jobs
        except Exception as e:
            logger.warn("taleo_fetch_jobs_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        job_url = raw_job.get("url") or f"https://{company_identifier}.taleo.net"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://{company_identifier}.taleo.net",
