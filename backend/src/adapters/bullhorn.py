...
class BullhornAdapter(BaseAdapter):
    platform = ATSPlatform.BULLHORN

    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch jobs from Bullhorn-powered staffing agency site."""
        base_domain = f"https://{company_identifier}.com" if not company_identifier.startswith("http") else company_identifier.rstrip('/')
        url = f"{base_domain}/jobs"
        try:
            ...
            jobs = []
            links = soup.select("a[href*='/job/'], a[href*='/jobs/'], a[href*='bullhorn']")
            for link in links:
                title = link.get_text(strip=True)
                href = link.get("href", "")
                if title and len(title) > 3:
                    full_url = href if href.startswith("http") else f"{base_domain}{href if href.startswith('/') else '/' + href}"
                    jobs.append({"id": full_url, "title": title, "url": full_url})
            return jobs
        except Exception as e:
            logger.warn("bullhorn_fetch_jobs_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        job_url = raw_job.get("url") or f"{base_domain}/jobs"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://{company_identifier}.com",
