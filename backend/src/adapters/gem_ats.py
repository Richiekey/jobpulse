...
class GemAdapter(BaseAdapter):
    platform = ATSPlatform.GEM

    async def fetch_jobs(self, company_identifier: str) -> List[Dict[str, Any]]:
        """Fetch jobs from Gem career page."""
        url = f"https://jobs.gem.com/{company_identifier}"
        try:
            ...
            jobs = []
            links = soup.select("a[href*='/job/'], a[href*='/jobs/'], a[href*='gem.com']")
            for link in links:
                ...
                if title and len(title) > 3:
                    jobs.append({
                        "id": href.split("/")[-1],
                        "title": title,
                        "url": href if href.startswith("http") else f"https://jobs.gem.com{href}"
                    })
            return jobs
        except Exception as e:
            logger.warn("gem_fetch_jobs_error", company=company_identifier, error=str(e))
            return []
    ...
    def normalize(self, raw_job: Dict[str, Any], company_name: str, company_identifier: str) -> NormalizedJob:
        ...
        job_url = raw_job.get("url") or f"https://jobs.gem.com/{company_identifier}"
        ...
        return NormalizedJob(
            ...
            company_url=f"https://jobs.gem.com/{company_identifier}",
